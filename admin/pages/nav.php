<nav>
    <div class="d-flex flex-column align-items-center py-3 px-2">
        ...
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#changePassword"><i class="fa-solid fa-pen-to-square"></i> Ubah Password
        </button>
    </div>
    <div class="kumpulanLink">
        ...
        <div class="page-catalogue">
            <a href="<?= $routing->getCataloguePage() ?>" class="text-light <?= $routing->isCatalogueActive() ? 'active' : '' ?>">Katalog</a>
        </div>
        <div class="page-socialmedia">
            <a href="<?= $routing->getSocialPage() ?>" class="text-light <?= $routing->isSocialActive() ? 'active' : '' ?>">Media Sosial</a>
        </div>
        <div class="page-lend">
            <a href="<?= $routing->getLendPage() ?>" class="text-light <?= $routing->isLendActive() ? 'active' : '' ?>">Data Peminjaman</a>
        </div>
        <div class="page-backweb">
            <a href="<?= $routing->getBackToWebPage() ?>" class="text-light <?= $routing->isBackWebActive() ? 'active' : '' ?>">Kembali ke Web</a>
        </div>
